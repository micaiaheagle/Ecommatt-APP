                            ...
                            >
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <div className="text-right shrink-0 ml-4">
                                        <span className={`font-bold text-sm whitespace-nowrap ${isOver ? 'text-red-600' : 'text-gray-900'}`}>${actual.toLocaleString()}</span>
                                        <span className="text-xs text-gray-400 whitespace-nowrap"> / ${budget.toLocaleString()}</span>
                                    </div>
                                </div>
                                            ...
                                            {actualData.items.map(item => (
                                                <div key={item.id} className="flex justify-between items-center text-xs">
                                                    <div className="flex-1 min-w-0 mr-4">
                                                        ...
                                                        <span className="text-gray-400 ml-2 whitespace-nowrap">{item.date}</span>
                                                    </div>
                                                    <span className="font-bold text-gray-900 shrink-0 whitespace-nowrap">${item.amount.toLocaleString()}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}

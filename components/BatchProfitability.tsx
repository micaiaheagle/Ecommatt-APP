                        ...
                        {batchRecords.map(rec => (
                            <div key={rec.id} className="bg-white p-3 rounded-xl border border-gray-100 flex justify-between items-center">
                                <div className="flex-1 min-w-0 mr-4">
                                    <p className="text-xs font-bold text-gray-800 truncate">{rec.description}</p>
                                    <p className="text-[10px] text-gray-500 truncate">{rec.date} • {rec.category}</p>
                                </div>
                                <span className={`text-xs font-bold shrink-0 whitespace-nowrap ${rec.type === 'Income' ? 'text-green-600' : 'text-red-500'}`}>
